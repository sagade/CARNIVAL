...
- Note: The default optimisation time is set to 600 seconds (10 minutes) which generally returns a first result for most of the stand-alone computers which were tested. Please sit back and relax while waiting for the results.
...
Beside the exported model descriptions and figures, users can also visualise the overall network results in an R-Shiny application with the function 'shinyCARNIVAL'. This function requires the shiny package (available on CRAN) and the rcytoscape package which can be download here: https://github.com/cytoscape/cyjShiny/releases. The results from the CARNIVAL example 2 can for instance be visualed on R-Shiny using the following command.
